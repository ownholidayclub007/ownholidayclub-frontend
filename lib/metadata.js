...
export function createPageMetadata({
  ...
  const canonicalUrl = canonical || buildAbsoluteUrl(path === "/" ? "" : path);
...
export async function getCombinedMetadata(path, fallbackData = {}) {
  ...
  if (dbSeo && dbSeo.isActive) {
    return createPageMetadata({
      path,
      absoluteTitle: dbSeo.metaTitle || staticMeta.absoluteTitle || staticMeta.title || fallbackData.absoluteTitle || fallbackData.title,
      description: dbSeo.metaDescription || staticMeta.description || fallbackData.description,
      image: dbSeo.ogImage || staticMeta.image || fallbackData.image,
      keywords: dbSeo.metaKeywords ? dbSeo.metaKeywords.split(",").map(k => k.trim()) : (fallbackData.keywords || undefined),
      canonical: dbSeo.canonicalTag || fallbackData.canonical || undefined,
      type: fallbackData.type || "website",
      publishedTime: fallbackData.publishedTime,
      modifiedTime: fallbackData.modifiedTime,
    });
  }

  if (STATIC_PAGE_METADATA[path]) {
  ...
  return createPageMetadata({
    ...
    absoluteTitle: fallbackData.absoluteTitle,
    description: fallbackData.description,
    image: fallbackData.image,
    keywords: fallbackData.keywords,
    noIndex: fallbackData.noIndex,
    canonical: fallbackData.canonical,
    type: fallbackData.type || "website",
    publishedTime: fallbackData.publishedTime,
    modifiedTime: fallbackData.modifiedTime,
  });
